...
onMounted(() => {
  ...
  // Fallback: If section is already visible (e.g., in viewport on page load), show it
  setTimeout(() => {
    const elements = ctaRef.value?.querySelectorAll(".animate-on-scroll");
    if (elements) {
      elements.forEach((el) => {
        ...
        if (isVisible && gsap.getProperty(el, "opacity") === 0) {
          gsap.to(el, {
            opacity: 1,
            y: 0,
            duration: 0.8,
...
.animate-on-scroll {
  /* Default visible state in case JS doesn't load */
  opacity: 1;
